...
A DLC pipeline provides a way for games' look and feel to be changed without a client update There are a number of reasons to avoid client updates, chief among them users are fickle and requiring updates before they see new content defeats the purpose of increasing engagement using content updates. 
...
1. __Testability:__
One requirement on content pipelines that people often forget is that Developers, Artists and Designers will all be testing changes to various assets concurrently. There should be a way for all of these individuals to test their changes without effecting other users. This is especially important to consider from the perspective of Artists and Designers who usually cannot build the game code locally. 

1. __Good Isolation:__
A related requirements is to be able to easily isolate the various environments (Dev, Integration, Staging, Production) from each other so that they can be tested independently. Further, developers, artists and designers should be able to test their changes independently of all other changes before committing them to source control.
...
Another important benefit of separating asset concepts from concrete implementations is to easily support localization and asset tiers. In the example index shown earlier note that some parts of the path are parenthesized. We define variables in the concrete implementation path that are resolved at run time. When the asset loader is initialized it is given (or detects) parameters such as the language and asset quality or even platform (iOS, Android, PC etc). Before downloading assets the manager will replace these variables with the actual language, asset quality and platform on which the game is running on. Hence the asset loader will only download the assets required for the given game client. 
...
{% codeblock  Index bloat with Language and Quality Tier lang:javascript %}
[
    {
        "concept": "main-loading-screen",
        "concrete-asset": [
